            ...
            <div class="row">
                    <?php 
                        if($_POST)
                        {
                            ...
                            $consulta = $pdo -> prepare ("INSERT INTO `tblventas` (`ID_TRANSACCION`, `CLAVE_TRANSACCION`, `PAYPAL_DATOS`, `FECHA`, `CORREO`, `TOTAL`, `ESTADO`) 
                            VALUES (NULL, :pCLAVE_TRANSACCION, '', NOW(), :pCORREO, :pTOTAL, 'PENDIENTE');");

                            $consulta->bindParam(":pCLAVE_TRANSACCION",$SID);
                            $consulta->bindParam(":pCORREO",$Correo);
                            $consulta->bindParam(":pTOTAL",$total);
                            ...
                            foreach($_SESSION['CARRITO'] as $indice => $producto){
                                $consulta = $pdo -> prepare("INSERT INTO `tbldetalleventa` (`ID_DETALLE`, `IDPRESEN`, `ID_TRANSACCION`, `PRECIO`, `CANTIDAD`)
                            VALUES (NULL,:IDPRESEN,:ID_TRANSACCION,:PRECIO,:CANTIDAD);");

                            $consulta->bindParam(":IDPRESEN",$producto['IDPH']);
                            $consulta->bindParam(":ID_TRANSACCION",$IDVenta);
                            $consulta->bindParam(":PRECIO",$producto['PRECIO']);
                    ...
                    <div class="jumbotron text-center">
                        <h1 class="display-4">Finalice su Pedido</h1>
                        <hr class="my-4">
                        <p class="lead">Está a punto de cancelar, el monto de:    
                            <h4><strong>₡<?php echo number_format($total,2);?></strong></h4>
                            <div id="paypal-button-container"></div>
                        </p>
                        <br>
                            <p><strong>Para consultas o sugerencias, contáctenos al correo: yarielamo@gmail.com o al teléfono 2573-7306</strong></p>   
                                ...
                                return actions.order.create({
                                    purchase_units: [{
                                        amount: {
                                            ...
                                            description: "Compra de Huevos a la Granja Avícola 1926:₡<?php echo number_format($total,2);?>"
                                        }
                                    }]
                                });
                            },
